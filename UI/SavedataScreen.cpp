...
void SavedataButton::Draw(UIContext &dc) {
	...
	if (texture) {
		...
		if (HasFocus()) {
			...
			draw->DrawImage4Grid(dc.theme->dropShadow4Grid, x - dropsize*1.5f, y - dropsize*1.5f, x + w + dropsize*1.5f, y + h + dropsize*1.5f, alphaMul(color, pulse), 1.0f);
			draw->Flush();
		} else {
			draw->Flush();
			dc.RebindTexture();
			draw->DrawImage4Grid(dc.theme->dropShadow4Grid, x - dropsize, y - dropsize*0.5f, x + w + dropsize, y + h + dropsize*1.5, alphaMul(shadowColor, 0.5f), 1.0f);
			draw->Flush();
		}
	}
