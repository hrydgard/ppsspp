			...
			if (component != ".")
			{
				if (component == "..")
				{
					...
					else
					{
						...
						WARN_LOG(Log::FILESYS, "RealPath: ignoring .. beyond root - root directory is its own parent: \"%s\"", pathString.c_str());
					}
				}
				else
				{
...
static bool RealPath(const std::string &currentDirectory, const std::string &inPath, std::string &outPath)
{
	...
	if (relative)
	{
		...
		if (curDirLen == 0)
		{
			ERROR_LOG(Log::FILESYS, "RealPath: inPath \"%s\" is relative, but current directory is empty", inPath.c_str());
			return false;
		}
		...
		{
			ERROR_LOG(Log::FILESYS, "RealPath: inPath \"%s\" is relative, but current directory \"%s\" has no prefix", inPath.c_str(), currentDirectory.c_str());
			return false;
		}
		if (curDirColon + 1 == curDirLen)
		{
			WARN_LOG(Log::FILESYS, "RealPath: inPath \"%s\" is relative, but current directory \"%s\" is all prefix and no path. Using \"/\" as path for current directory.", inPath.c_str(), currentDirectory.c_str());
		}
		else
		{
			...
			{
				ERROR_LOG(Log::FILESYS,"RealPath: currentDirectory is not a valid path: \"%s\"", currentDirectory.c_str());
				return false;
			}
	...
	{
		WARN_LOG(Log::FILESYS, "RealPath: inPath is not a valid path: \"%s\"", inPath.c_str());
		return false;
	}
...
int MetaFileSystem::MapFilePath(const std::string &_inpath, std::string &outpath, MountPoint **system)
{
	...
	if (strncasecmp(inpath.c_str(), "host0:", strlen("host0:")) == 0) {
		INFO_LOG(Log::FILESYS, "Host0 path detected, stripping: %s", inpath.c_str());
	...
	if (it == currentDir.end()) 
	{
		...
		if (inpath.find(':') == std::string::npos /* means path is relative */) 
		{
			...
			WARN_LOG(Log::FILESYS, "Path is relative, but current directory not set for thread %i. returning 8002032C(SCE_KERNEL_ERROR_NOCWD) instead.", currentThread);
		}
	}
	else
	{
		...
		for (size_t i = 0; i < fileSystems.size(); i++)
		{
			...
			if (strncasecmp(fileSystems[i].prefix.c_str(), prefix.c_str(), prefLen) == 0)
			{
				...
				*system = &(fileSystems[i]);
				...
				VERBOSE_LOG(Log::FILESYS, "MapFilePath: mapped \"%s\" to prefix: \"%s\", path: \"%s\"", inpath.c_str(), fileSystems[i].prefix.c_str(), outpath.c_str());
	...
	DEBUG_LOG(Log::FILESYS, "MapFilePath: failed mapping \"%s\", returning false", inpath.c_str());
	return error;
}
	...
	else
	{
		for (size_t i = 0; i < fileSystems.size(); i++)
		{
			...
			if (strncasecmp(prefix.c_str(), dir.c_str(), prefix.size()) == 0)
			{
				...
				WARN_LOG(Log::FILESYS, "ChDir failed to map path \"%s\", saving as current directory anyway", dir.c_str());
				currentDir[curThread] = dir;
				return 0;
			}
		}

		WARN_LOG_REPORT(Log::FILESYS, "ChDir failed to map device for \"%s\", failing", dir.c_str());
...
void MetaFileSystem::DoState(PointerWrap &p)
{
	...
	if (n != (u32) fileSystems.size())
	{
		if (n == (u32) fileSystems.size() - 1) {
			...
			ERROR_LOG(Log::FILESYS, "Savestate failure: number of filesystems doesn't match.");
			return;
		}
	}
