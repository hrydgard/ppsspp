...
void VirtualDiscFileSystem::LoadFileListIndex() {
	...
	while (fgets(linebuf, MAX_LINE_SIZE, f)) {
		...
		if (filename_pos == line.npos) {
			ERROR_LOG(Log::FILESYS, "Unexpected line in %s: %s", INDEX_FILENAME.c_str(), line.c_str());
			continue;
		}
		...
		if (entry.handler != NULL && entry.handler->IsValid()) {
			...
			if (temp.Open(basePath.ToString(), entry.fileName, FILEACCESS_READ)) {
				...
				ERROR_LOG(Log::FILESYS, "Unable to open virtual file: %s", entry.fileName.c_str());
			}
		} else {
	...
	if (p.mode == p.MODE_READ)
	{
		...
		for (int i = 0; i < entryCount; i++)
		{
			...
			if (of.type != VFILETYPE_ISO) {
				...
				if (!success) {
					ERROR_LOG(Log::FILESYS, "Failed to create file handle for %s.", fileList[of.fileIndex].fileName.c_str());
				} else {
...
int VirtualDiscFileSystem::OpenFile(std::string filename, FileAccess access, const char *devicename)
{
	...
	if (filename.compare(0, 8, "/sce_lbn") == 0)
	{
		...
		if (fileIndex == -1)
		{
			ERROR_LOG(Log::FILESYS, "VirtualDiscFileSystem: sce_lbn used without calling fileinfo.");
			return 0;
		}
...
#ifdef _WIN32
				ERROR_LOG(Log::FILESYS, "VirtualDiscFileSystem::OpenFile: FAILED, %i", (int)GetLastError());
#else
				ERROR_LOG(Log::FILESYS, "VirtualDiscFileSystem::OpenFile: FAILED");
#endif
			}
			return 0;
		}
	...
	if (!success) {
		if (!(access & FILEACCESS_PPSSPP_QUIET)) {
#ifdef _WIN32
			ERROR_LOG(Log::FILESYS, "VirtualDiscFileSystem::OpenFile: FAILED, %i - access = %i", (int)GetLastError(), (int)access);
#else
			ERROR_LOG(Log::FILESYS, "VirtualDiscFileSystem::OpenFile: FAILED, access = %i", (int)access);
#endif
		}
...
size_t VirtualDiscFileSystem::SeekFile(u32 handle, s32 position, FileMove type) {
	...
	if (iter != entries.end()) {
		...
		ERROR_LOG(Log::FILESYS,"VirtualDiscFileSystem: Cannot seek in file that hasn't been opened: %08x", handle);
		return 0;
	}
}
...
size_t VirtualDiscFileSystem::ReadFile(u32 handle, u8 *pointer, s64 size, int &usec) {
	...
	if (iter != entries.end()) {
		if (size < 0) {
			ERROR_LOG_REPORT(Log::FILESYS, "Invalid read for %lld bytes from virtual umd", size);
			return 0;
		}
		...
		if (iter->second.type == VFILETYPE_ISO)
		{
			...
			if (fileIndex == -1)
			{
				ERROR_LOG(Log::FILESYS,"VirtualDiscFileSystem: Reading from unknown address in %08x at %08llx", handle, iter->second.curOffset);
				return 0;
			}
			...
			if (!success)
			{
				ERROR_LOG(Log::FILESYS,"VirtualDiscFileSystem: Error opening file %s", fileList[fileIndex].fileName.c_str());
				return 0;
			}
		...
		if (iter->second.type == VFILETYPE_LBN && iter->second.curOffset + size > iter->second.size) {
			...
			WARN_LOG(Log::FILESYS, "VirtualDiscFileSystem: Reading beyond end of file, clamping size %lld to %lld", size, newSize);
			size = newSize;
		}
		...
		//This shouldn't happen...
		ERROR_LOG(Log::FILESYS,"VirtualDiscFileSystem: Cannot read file that hasn't been opened: %08x", handle);
		return 0;
	}
}
...
void VirtualDiscFileSystem::CloseFile(u32 handle) {
	...
	if (iter != entries.end()) {
		...
		//This shouldn't happen...
		ERROR_LOG(Log::FILESYS,"VirtualDiscFileSystem: Cannot close file that hasn't been opened: %08x", handle);
	}
}
...
PSPFileInfo VirtualDiscFileSystem::GetFileInfo(std::string filename) {
	...
	if (x.type != FILETYPE_DIRECTORY) {
		...
		if (!File::GetFileInfo(fullName, &details)) {
			ERROR_LOG(Log::FILESYS, "DirectoryFileSystem::GetFileInfo: GetFileInfo failed: %s", fullName.c_str());
			x.size = 0;
			x.access = 0;
		} else {
			x.size = details.size;
...
std::vector<PSPFileInfo> VirtualDiscFileSystem::GetDirListing(const std::string &path, bool *exists) {
	...
	if (dp == NULL) {
		ERROR_LOG(Log::FILESYS,"Error opening directory %s\n", path.c_str());
		if (exists)
			*exists = false;
		return myVector;
	}
...
{
	ERROR_LOG(Log::FILESYS,"VirtualDiscFileSystem: Cannot write to file on virtual disc");
	return 0;
}
...
{
	ERROR_LOG(Log::FILESYS,"VirtualDiscFileSystem: Cannot create directory on virtual disc");
	return false;
}

bool VirtualDiscFileSystem::RmDir(const std::string &dirname)
{
	ERROR_LOG(Log::FILESYS,"VirtualDiscFileSystem: Cannot remove directory on virtual disc");
	return false;
}

int VirtualDiscFileSystem::RenameFile(const std::string &from, const std::string &to)
{
	ERROR_LOG(Log::FILESYS,"VirtualDiscFileSystem: Cannot rename file on virtual disc");
	return -1;
}

bool VirtualDiscFileSystem::RemoveFile(const std::string &filename)
{
	ERROR_LOG(Log::FILESYS,"VirtualDiscFileSystem: Cannot remove file on virtual disc");
	return false;
}
...
#define dlclose(mod) FreeLibrary((HMODULE)mod)
	...
	if (library != NULL) {
		...
		if (!Init || !Shutdown || !Open || !Seek || !Read || !Close) {
			ERROR_LOG(Log::FILESYS, "Unable to find all handler functions: %s", filename);
			dlclose(library);
			library = NULL;
		} else if (!Init(&HandlerLogger, sys)) {
			ERROR_LOG(Log::FILESYS, "Unable to initialize handler: %s", filename);
			dlclose(library);
			library = NULL;
		}
	} else {
		ERROR_LOG(Log::FILESYS, "Unable to load handler '%s': %s", filename, GetLastErrorMsg().c_str());
	}
#ifdef _WIN32
#undef dlopen
#undef dlsym
