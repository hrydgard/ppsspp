...
bool DirectoryFileHandle::Open(const Path &basePath, std::string &fileName, FileAccess access, u32 &error) {
	...
	if (access & (FILEACCESS_APPEND | FILEACCESS_CREATE | FILEACCESS_WRITE)) {
		DEBUG_LOG(Log::FILESYS, "Checking case for path %s", fileName.c_str());
	...
	if (fullName.Type() == PathType::CONTENT_URI) {
		...
		if (fd != -1) {
			...
			ERROR_LOG(Log::FILESYS, "File::OpenFD returned an error");
	...
	if (!success && !(access & FILEACCESS_CREATE)) {
		...
		DEBUG_LOG(Log::FILESYS, "Case may have been incorrect, second try opening %s (%s)", fullName.c_str(), fileName.c_str());
...
size_t DirectoryFileHandle::Write(const u8* pointer, s64 size)
{
	...
	if (diskFull) {
		ERROR_LOG(Log::FILESYS, "Disk full");
		auto err = GetI18NCategory(I18NCat::ERRORS);
		g_OSD.Show(OSDType::MESSAGE_ERROR, err->T("Disk full while writing data"));
...
void DirectoryFileHandle::Close()
{
	if (needsTrunc_ != -1) {
		...
		if (SetEndOfFile(hFile) == 0) {
			ERROR_LOG_REPORT(Log::FILESYS, "Failed to truncate file.");
		}
...
void DirectoryFileSystem::CloseAll() {
	for (auto iter = entries.begin(); iter != entries.end(); ++iter) {
		INFO_LOG(Log::FILESYS, "DirectoryFileSystem::CloseAll(): Force closing %d (%s)", (int)iter->first, iter->second.guestFilename.c_str());
...
int DirectoryFileSystem::OpenFile(std::string filename, FileAccess access, const char *devicename) {
	...
	if (err != 0) {
		...
		if (!(access & FILEACCESS_PPSSPP_QUIET)) {
			ERROR_LOG(Log::FILESYS, "DirectoryFileSystem::OpenFile('%s'): FAILED, %d - access = %d '%s'", filename.c_str(), logError, (int)(access & FILEACCESS_PSP_FLAGS), errorString.c_str());
		}
		return err;
	} else {
#ifdef _WIN32
...
void DirectoryFileSystem::CloseFile(u32 handle) {
	...
	if (iter != entries.end()) {
		...
		ERROR_LOG(Log::FILESYS,"Cannot close file that hasn't been opened: %08x", handle);
	}
}
...
size_t DirectoryFileSystem::ReadFile(u32 handle, u8 *pointer, s64 size, int &usec) {
	...
	if (iter != entries.end()) {
		if (size < 0) {
			ERROR_LOG_REPORT(Log::FILESYS, "Invalid read for %lld bytes from disk %s", size, iter->second.guestFilename.c_str());
			return 0;
		}
		...
		// This shouldn't happen...
		ERROR_LOG(Log::FILESYS,"Cannot read file that hasn't been opened: %08x", handle);
		return 0;
	}
}
...
size_t DirectoryFileSystem::WriteFile(u32 handle, const u8 *pointer, s64 size, int &usec) {
	...
	if (iter != entries.end()) {
		...
		//This shouldn't happen...
		ERROR_LOG(Log::FILESYS,"Cannot write to file that hasn't been opened: %08x", handle);
		return 0;
	}
}
...
size_t DirectoryFileSystem::SeekFile(u32 handle, s32 position, FileMove type) {
	...
	if (iter != entries.end()) {
		...
		//This shouldn't happen...
		ERROR_LOG(Log::FILESYS,"Cannot seek in file that hasn't been opened: %08x", handle);
		return 0;
	}
}
...
static std::string SimulateVFATBug(std::string filename) {
	...
	if (apply_hack) {
		VERBOSE_LOG(Log::FILESYS, "Applying VFAT hack to filename: %s", filename.c_str());
	...
	if (p.mode == p.MODE_READ) {
		...
		for (u32 i = 0; i < num; i++) {
			...
			if (!entry.hFile.Open(basePath,entry.guestFilename,entry.access, err)) {
				ERROR_LOG(Log::FILESYS, "Failed to reopen file while loading state: %s", entry.guestFilename.c_str());
				brokenFile = true;
			}
			u32 position;
			Do(p, position);
			if (position != entry.hFile.Seek(position, FILEMOVE_BEGIN)) {
				ERROR_LOG(Log::FILESYS, "Failed to restore seek position while loading state: %s", entry.guestFilename.c_str());
				brokenFile = true;
			}
			if (s >= 2) {
...
int VFSFileSystem::OpenFile(std::string filename, FileAccess access, const char *devicename) {
	if (access != FILEACCESS_READ) {
		ERROR_LOG(Log::FILESYS, "VFSFileSystem only supports plain reading");
		return SCE_KERNEL_ERROR_ERRNO_INVALID_FLAG;
	}

	std::string fullName = GetLocalPath(filename);
	const char *fullNameC = fullName.c_str();
	VERBOSE_LOG(Log::FILESYS, "VFSFileSystem actually opening %s (%s)", fullNameC, filename.c_str());

	size_t size;
	u8 *data = g_VFS.ReadFile(fullNameC, &size);
	if (!data) {
		ERROR_LOG(Log::FILESYS, "VFSFileSystem failed to open %s", filename.c_str());
...
size_t VFSFileSystem::ReadFile(u32 handle, u8 *pointer, s64 size, int &usec) {
	DEBUG_LOG(Log::FILESYS,"VFSFileSystem::ReadFile %08x %p %i", handle, pointer, (u32)size);
	EntryMap::iterator iter = entries.find(handle);
	if (iter != entries.end())
	{
...
void VFSFileSystem::DoState(PointerWrap &p) {
	...
	if (num != 0) {
		...
		ERROR_LOG(Log::FILESYS, "FIXME: Open files during savestate, could go badly.");
	}
}
