...
bool FileBlockDevice::ReadBlock(int blockNumber, u8 *outPtr, bool uncached) {
	...
	if (retval != 2048) {
		DEBUG_LOG(Log::FILESYS, "Could not read 2048 byte block, at block offset %d. Only got %d bytes", blockNumber, (int)retval);
		return false;
	}
...
bool FileBlockDevice::ReadBlocks(u32 minBlock, int count, u8 *outPtr) {
	...
	if (retval != (size_t)count) {
		ERROR_LOG(Log::FILESYS, "Could not read %d blocks, at block offset %d. Only got %d blocks", count, minBlock, (int)retval);
		return false;
	}
	return true;
}
