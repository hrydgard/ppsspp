...
ISOFileSystem::ISOFileSystem(IHandleAllocator *_hAlloc, BlockDevice *_blockDevice) {
	...
	if (memcmp(desc.cd001, "CD001", 5)) {
		ERROR_LOG(Log::FILESYS, "ISO looks bogus, expected CD001 signature not present? Giving up...");
		return;
	}
...
void ISOFileSystem::ReadDirectory(TreeEntry *root) {
	for (u32 secnum = root->startsector, endsector = root->startsector + (root->dirsize + 2047) / 2048; secnum < endsector; ++secnum) {
		...
		if (!blockDevice->ReadBlock(secnum, theSector)) {
			blockDevice->NotifyReadError();
			ERROR_LOG(Log::FILESYS, "Error reading block for directory '%s' in sector %d - skipping", root->name.c_str(), secnum);
		...
		for (int offset = 0; offset < 2048; ) {
			...
			if (offset + IDENTIFIER_OFFSET + dir.identifierLength > 2048) {
				blockDevice->NotifyReadError();
				ERROR_LOG(Log::FILESYS, "Directory entry crosses sectors, corrupt iso?");
				return;
			}
			...
			VERBOSE_LOG(Log::FILESYS, "%s: %s %08x %08x %d", entry->isDirectory ? "D" : "F", entry->name.c_str(), (u32)dir.firstDataSector, entry->startingPosition, entry->startingPosition);

			// Round down to avoid any false reports.
			if (isFile && dir.firstDataSector + (dir.dataLength / 2048) > blockDevice->GetNumBlocks()) {
				blockDevice->NotifyReadError();
				ERROR_LOG(Log::FILESYS, "File '%s' starts or ends outside ISO. firstDataSector: %d len: %d", entry->BuildPath().c_str(), (int)dir.firstDataSector, (int)dir.dataLength);
			}

			if (entry->isDirectory && !relative) {
				if (entry->startsector == root->startsector) {
					blockDevice->NotifyReadError();
					ERROR_LOG(Log::FILESYS, "WARNING: Appear to have a recursive file system, breaking recursion. Probably corrupt ISO.");
				}
			}
...
ISOFileSystem::TreeEntry *ISOFileSystem::GetFromPath(const std::string &path, bool catchError) {
	...
	while (true) {
		...
		if (nextEntry) {
			...
			if (catchError)
				ERROR_LOG(Log::FILESYS, "File '%s' not found", path.c_str());
...
int ISOFileSystem::OpenFile(std::string filename, FileAccess access, const char *devicename) {
	...
	if (access & FILEACCESS_WRITE) {
		ERROR_LOG(Log::FILESYS, "Can't open file '%s' with write access on an ISO partition", filename.c_str());
	...
	if (filename.compare(0, 8, "/sce_lbn") == 0) {
		...
		if (sectorStart > blockDevice->GetNumBlocks()) {
			WARN_LOG(Log::FILESYS, "Unable to open raw sector, out of range: '%s', sector %08x, max %08x", filename.c_str(), sectorStart, blockDevice->GetNumBlocks());
			return SCE_KERNEL_ERROR_ERRNO_FILE_NOT_FOUND;
		}
		else if (sectorStart == blockDevice->GetNumBlocks())
		{
			ERROR_LOG(Log::FILESYS, "Should not be able to open the block after the last on disc! %08x", sectorStart);
		}

		DEBUG_LOG(Log::FILESYS, "Got a raw sector open: '%s', sector %08x, size %08x", filename.c_str(), sectorStart, readSize);
...
void ISOFileSystem::CloseFile(u32 handle) {
	...
	if (iter != entries.end()) {
		...
		ERROR_LOG(Log::FILESYS, "Hey, what are you doing? Closing non-open files?");
	}
}
...
int ISOFileSystem::Ioctl(u32 handle, u32 cmd, u32 indataPtr, u32 inlen, u32 outdataPtr, u32 outlen, int &usec) {
	EntryMap::iterator iter = entries.find(handle);
	if (iter == entries.end()) {
		ERROR_LOG(Log::FILESYS, "Ioctl on a bad file handle");
	...
	case 0x01020001:
		if (e.isBlockSectorMode) {
			ERROR_LOG(Log::FILESYS, "Unsupported read volume descriptor command on a umd block device");
			return SCE_KERNEL_ERROR_ERRNO_FUNCTION_NOT_SUPPORTED;
		}

		if (!Memory::IsValidRange(outdataPtr, 0x800) || outlen < 0x800) {
			WARN_LOG_REPORT(Log::FILESYS, "sceIoIoctl: Invalid out pointer %08x while reading ISO9660 volume descriptor", outdataPtr);
	...
	case 0x01020002:
		if (e.isBlockSectorMode) {
			ERROR_LOG(Log::FILESYS, "Unsupported read path table command on a umd block device");
			return SCE_KERNEL_ERROR_ERRNO_FUNCTION_NOT_SUPPORTED;
		}
...
size_t ISOFileSystem::ReadFile(u32 handle, u8 *pointer, s64 size, int &usec) {
	...
	if (iter != entries.end()) {
		...
		if (size < 0) {
			ERROR_LOG_REPORT(Log::FILESYS, "Invalid read for %lld bytes from umd %s", size, e.file ? e.file->name.c_str() : "device");
			return 0;
		}
		...
		if (e.isRawSector) {
			...
			ERROR_LOG(Log::FILESYS, "File no longer exists (loaded savestate with different ISO?)");
			return 0;
		} else {
		...
		if ((s64)e.seekPos > fileSize) {
			WARN_LOG(Log::FILESYS, "Read starting outside of file, at %lld / %lld", (s64)e.seekPos, fileSize);
			return 0;
		}
		if ((s64)e.seekPos + size > fileSize) {
			...
			if (newSize == 0) {
				INFO_LOG(Log::FILESYS, "Attempted read at end of file, 0-size read simulated");
			} else {
				INFO_LOG(Log::FILESYS, "Reading beyond end of file from seekPos %d, clamping size %lld to %lld", e.seekPos, size, newSize);
			}
			size = newSize;
		}
		...
		if ((middleSize & 2047) != 0) {
			ERROR_LOG(Log::FILESYS, "Remaining size should be aligned");
		}
		...
		//This shouldn't happen...
		ERROR_LOG(Log::FILESYS, "Hey, what are you doing? Reading non-open files?");
		return 0;
	}
}

size_t ISOFileSystem::WriteFile(u32 handle, const u8 *pointer, s64 size) {
	ERROR_LOG(Log::FILESYS, "Hey, what are you doing? You can't write to an ISO!");
	return 0;
}
...
size_t ISOFileSystem::SeekFile(u32 handle, s32 position, FileMove type) {
	...
	if (iter != entries.end()) {
		...
		//This shouldn't happen...
		ERROR_LOG(Log::FILESYS, "Hey, what are you doing? Seeking in non-open files?");
		return 0;
	}
}
