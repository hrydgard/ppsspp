...
LocalFileLoader::LocalFileLoader(const Path &filename)
	...
	if (filename.empty()) {
		ERROR_LOG(Log::FILESYS, "LocalFileLoader can't load empty filenames");
		return;
	}
	...
	if (filename.Type() == PathType::CONTENT_URI) {
		...
		if (fd < 0) {
			ERROR_LOG(Log::FILESYS, "LocalFileLoader failed to open content URI: '%s'", filename.c_str());
			return;
		}
		fd_ = fd;
		isOpenedByFd_ = true;
...
size_t LocalFileLoader::ReadAt(s64 absolutePos, size_t bytes, size_t count, void *data, Flags flags) {
	...
	if (filesize_ == 0) {
		ERROR_LOG(Log::FILESYS, "ReadAt from 0-sized file: %s", filename_.c_str());
		return 0;
	}
