...
namespace MIPSDis
{
	...
	void Dis_FPULS(u32 op, char *out)
	{
		...
		int rs = _RS;
		const char *name = MIPSGetName(op);
		sprintf(out, "%s\t%s, %X(%s)",name,FN(ft),offset,RN(rs));
	}
	...
	void Dis_IType(u32 op, char *out)
	{
		...
		int rs = _RS;
		const char *name = MIPSGetName(op);
		sprintf(out, "%s\t%s, %s, %X",name,RN(rt),RN(rs),imm);
	}
	...
	void Dis_IType1(u32 op, char *out)
	{
		...
		int rt = _RT;
		const char *name = MIPSGetName(op);
		sprintf(out, "%s\t%s, %X",name,RN(rt),imm);
	}
	...
	void Dis_ITypeMem(u32 op, char *out)
	{
		...
		int rs = _RS;
		const char *name = MIPSGetName(op);
		sprintf(out, "%s\t%s, %X(%s)",name,RN(rt),imm,RN(rs));
	}
	...
	void Dis_ShiftType(u32 op, char *out)
	{
		...
		if (((op & 0x3f) == 6) && sa == 1)
			name = "rotrv";
		sprintf(out, "%s\t%s, %s, %X",name,RN(rd),RN(rt),sa);
	}
	...
	void Dis_Special3(u32 op, char *out)
	{
		...
		case 0x0: //ext
			{
				int size = _SIZE + 1;
				sprintf(out,"%s\t%s, %s, %X, %X",name,RN(Rt),RN(rs),pos,size);
			}
			break;
		case 0x4: // ins
			{
