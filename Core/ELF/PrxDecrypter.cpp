...
// The same family for the older firmware generations, read out of each firmware's own
// mesg_led_XXg.prx: that tag table is 24-byte entries of tag plus 16-byte key. Extracting the
// 6.6x triple this way reproduces keys_9DC14891_1/2/3 above byte for byte, which is what
// establishes the layout. Each generation has one key per PSP model, and flash0:/vsh/etc/
// index_XXg.dat - the index of what the XMB shows - is tagged with the one for its model, so a
// shell whose key is missing decrypts nothing, draws no icons and gives up with the red error
// screen. Names are <generation>_<model>.
// 3.0x and 3.5x predate the per-model split and have a single flash0:/vsh/etc/index.dat.
...
// 2.6x and 2.7x tag their index.dat 0x495BE403 rather than with the 0x0B2Bxxx0 the rest of this
// family uses. The scramble code makes no difference to this file - every value tried decrypts it
// to the same plaintext - so it keeps the 0x5C the neighbouring entries have.
