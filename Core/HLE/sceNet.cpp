...
{
	DEBUG_LOG(SCENET, "sceNetGetDropRate %08x,%08x", dropRateAddr, dropDurationAddr);
	Memory::Write_U32(netDropRate, dropRateAddr);
	Memory::Write_U32(netDropDuration, dropDurationAddr);
	return 0;
}

static int sceNetSetDropRate(u32 dropRateAddr, u32 dropDurationAddr)
{
	DEBUG_LOG(SCENET, "sceNetSetDropRate %08x,%08x", dropRateAddr, dropDurationAddr);
	netDropRate = dropRateAddr;
	netDropDuration = dropDurationAddr;
	return 0;
}
...
const HLEFunction sceNetIfhandle[] = {
	...
	{ 0xFD8585E1, &WrapI_UU<sceNetSetDropRate>,          "sceNetUpnpStart",              'i', "ii" },
