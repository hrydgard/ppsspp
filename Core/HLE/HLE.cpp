...
void hleFlushCalls() {
	...
	for (int i = (int)enqueuedMipsCalls.size() - 1; i >= 0; --i) {
		...
		stackData->vals.func = info.func;
		if (info.action) {
			stackData->vals.actionIndex = (int)mipsCallActions.size();
			mipsCallActions.push_back(info.action);
		} else {
			stackData->vals.actionIndex = 0xFFFFFFFF;
		}
		stackData->vals.argc = (int)info.args.size();
...
void HLEReturnFromMipsCall() {
	...
	hleSkipDeadbeef();
	currentMIPS->pc = stackData->vals.func;
	currentMIPS->r[MIPS_REG_RA] = HLEMipsCallReturnAddress();
	for (int i = 0; i < (int)stackData->vals.argc; i++) {
