...
// GL entry points for the Switch (libnx), which ships no GLEW.
//
// The declarations below are generated, and match the symbol map in
// libretro/glsym/glsym_gl.c; LibretroGLContext::CreateDrawContext resolves them
// through eglGetProcAddress with rglgen at startup. Everything is redirected to
// the __gl* pointers by the defines at the end, so call sites keep spelling the
// plain GL names.
//
// Only GLCommon.h includes this, and only under PPSSPP_PLATFORM(SWITCH) - every
// other GL target keeps GLEW.
