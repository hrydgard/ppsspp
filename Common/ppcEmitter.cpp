...
namespace PpcGen {
	...
	void PPCXEmitter::B (const void *fnptr) {
		s32 func =  (intptr_t)fnptr - intptr_t(code);
...
#define IS_SMALL_JUMP (((uintptr_t)code - (uintptr_t)fnptr)>=-32767 && ((uintptr_t)code - (uintptr_t)fnptr)<=-32767)
#define CHECK_SMALL_JUMP { if(IS_SMALL_JUMP) { DebugBreak(); } }
	...
	{
		s32 distance =  intptr_t(code) - (intptr_t)branch.ptr;
		_assert_msg_(distance > -32767
			&& distance <=  32767,
		...
		default:
			// Error !!!
			_assert_msg_(0, "SetJumpTarget unknow branch type: %d", branch.type);
			break;
		}
	}
	...
	// Others ...
	void PPCXEmitter::SetCodePointer(u8 *ptr, u8 *writePtr)
	{
		code = ptr;
		startcode = code;
	...
	const u8 *PPCXEmitter::GetCodePointer() const
	{
		return code;
	}
	...
	void PPCXEmitter::FlushIcacheSection(u8 *start, u8 *end)
	{
		...
		__asm__ volatile (".long 0x7c0004ac");//sync
		__asm__ volatile (".long 0x4C00012C");//isync
#else
		while(addr < end) {
			__asm dcbst r0, addr
			__asm icbi r0, addr
	...
	void PPCXCodeBlock::PoisonMemory(int offset) {
		memset(region + offset, 0x00, region_size - offset);
	}
