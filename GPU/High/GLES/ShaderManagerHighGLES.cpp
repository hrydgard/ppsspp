...
void LinkedShader::UpdateUniforms(u32 vertType) {
	...
	if (dirty & DIRTY_PROJMATRIX) {
		...
		if (g_Config.bDepthRangeHack) {
			...
			float zOff = gstate.getViewportZCenter() / 65535.0f;
...
ShaderManagerGLES::ShaderManagerGLES() : lastShader_(NULL), globalDirty_(0xFFFFFFFF), shaderSwitchDirty_(0) {
...
bool ShaderManagerGLES::DebugAreShadersCompatibleForLinking(Shader *vs, Shader *fs) {
