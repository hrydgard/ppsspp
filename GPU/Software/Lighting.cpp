...
void Process(VertexData& vertex)
{
	...
	Vec3<int> mac = gstate.isMaterialAmbientEnabled()
						? vertex.color0.rgb()
	...
	for (unsigned int light = 0; light < 4; ++light) {
		...
		Vec3<int> mdc = gstate.isMaterialDiffuseEnabled()
							? vertex.color0.rgb()
		...
		if (gstate.isUsingSpecularLight(light)) {
			...
			Vec3<int> msc = gstate.isMaterialSpecularEnabled()
								? vertex.color0.rgb()
	...
	int maa = gstate.isMaterialAmbientEnabled() ? vertex.color0.a() : gstate.getMaterialAmbientA();
