...
void Process(VertexData& vertex)
{
	...
	Vec3<int> mac = gstate.isMaterialAmbientColor()
						? vertex.color0.rgb()
	...
	for (unsigned int light = 0; light < 4; ++light) {
		...
		Vec3<int> mdc = gstate.isMaterialDiffuseColor()
							? vertex.color0.rgb()
		...
		if (gstate.isUsingSpecularLight(light)) {
			...
			Vec3<int> msc = gstate.isMaterialSpecularColor()
								? vertex.color0.rgb()
	...
	int maa = gstate.isMaterialAmbientColor() ? vertex.color0.a() : gstate.getMaterialAmbientA();
