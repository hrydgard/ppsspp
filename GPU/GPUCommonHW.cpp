...
void GPUCommonHW::Execute_Prim(u32 op, u32 diff) {
	...
	if (gstate.isModeThrough()) {
		...
		cyclesExecuted += EstimateVideoBlitCycles(prim, verts, inds, count, decoder, vertexType);
	}
		...
		case GE_CMD_PRIM:
		{
			...
			if (passCulling) {
				if (vertexType & GE_VTYPE_THROUGH_MASK) {
					cyclesExecuted += EstimateVideoBlitCycles(newPrim, verts, inds, count, decoder, vertexType);
				}
