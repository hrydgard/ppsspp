...
float predictHorizontal(ivec2 base, int channel) {
	vec4 samples[8];
#define R(dx, dy) readComponent(base + ivec2(dx, dy), channel)
	samples[0] = vec4(R(-1, -3), R(-1, -2), R(-1, -1), R(-1, 0));
	samples[1] = vec4(R(-1, 1), R(-1, 2), R(-1, 3), R(-1, 4));
	samples[2] = vec4(R(0, -3), R(0, -2), R(0, -1), R(0, 0));
	samples[3] = vec4(R(0, 1), R(0, 2), R(0, 3), R(0, 4));
	samples[4] = vec4(R(1, -3), R(1, -2), R(1, -1), R(1, 0));
	samples[5] = vec4(R(1, 1), R(1, 2), R(1, 3), R(1, 4));
	samples[6] = vec4(R(2, -3), R(2, -2), R(2, -1), R(2, 0));
	samples[7] = vec4(R(2, 1), R(2, 2), R(2, 3), R(2, 4));
#undef R
...
void main() {
	...
	ivec2 srcPos = ivec2(dst.x >> 1, dst.y);
	if ((dst.x & 1) == 0) {
	...
	for (int channel = 0; channel < 4; ++channel) {
		color[channel] = predictHorizontal(srcPos, channel);
	}
