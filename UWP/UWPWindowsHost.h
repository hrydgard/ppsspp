...
class UWPWindowsHost : public Host {
public:
	UWPWindowsHost(Windows::UI::Xaml::Controls::SwapChainPanel^ window);
  ~UWPWindowsHost();
  ...
  HANDLE resizeLock = INVALID_HANDLE_VALUE;

private:
	std::list<std::shared_ptr<InputDevice>> input;

  Windows::UI::Xaml::Controls::SwapChainPanel^ window;
};
