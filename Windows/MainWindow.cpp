	...
	LRESULT CALLBACK DisplayProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
		...
		case WM_LBUTTONDOWN:
				...
				(GetMessageExtraInfo() & MOUSEEVENTF_MASK_PLUS_PENTOUCH) != MOUSEEVENTF_FROMTOUCH_NOPEN)
			{
				...
				float x = GET_X_LPARAM(lParam) * g_display.g_dpi_scale_x;
				float y = GET_Y_LPARAM(lParam) * g_display.g_dpi_scale_y;
		...
		case WM_MOUSEMOVE:
				...
				(GetMessageExtraInfo() & MOUSEEVENTF_MASK_PLUS_PENTOUCH) != MOUSEEVENTF_FROMTOUCH_NOPEN)
			{
				...
				float x = (float)cursorX * g_display.g_dpi_scale_x;
				float y = (float)cursorY * g_display.g_dpi_scale_y;
				WindowsRawInput::SetMousePos(x, y);
		...
		case WM_LBUTTONUP:
				...
				(GetMessageExtraInfo() & MOUSEEVENTF_MASK_PLUS_PENTOUCH) != MOUSEEVENTF_FROMTOUCH_NOPEN)
			{
				...
				float x = (float)GET_X_LPARAM(lParam) * g_display.g_dpi_scale_x;
				float y = (float)GET_Y_LPARAM(lParam) * g_display.g_dpi_scale_y;
				WindowsRawInput::SetMousePos(x, y);
