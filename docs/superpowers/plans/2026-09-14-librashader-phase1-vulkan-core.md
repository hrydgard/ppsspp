...
| twopass pixel diff | PASS (after the F1 fix; **FAIL before it**) | Originally 83.4% of pixels differ, max 121, mean 60.3 - librashader's `OriginalHistory1` snapshot held only the native-sized top-left corner of the upscaled source, stretched to full screen. Fixed by detecting `OriginalHistory[1-9]`/`OriginalHistorySize[1-9]` in the preset's (include-resolved) pass sources at load time and, for those presets only, blitting the source into a native-sized intermediate so the declared `libra_image_vk_t` size equals the real extents. Re-measured 2026-09-14 with the fix: **19.87% of pixels differ, max 120, mean |diff| 1.71/255 over the whole frame, mean signed -0.15/-0.13/-0.12** and a shift search whose clear minimum is (0,0) (1.71 vs 2.17-2.30 at +/-1 px), i.e. full-frame and phase-exact. The residual is only the *resolution* of the history term: librashader's history ring is snapshotted at the declared native size, so the 50%-weight `OriginalHistory1` sample is a native-res upscale where the in-tree chain keeps a full-res copy. Visually indistinguishable (`/tmp/ppsspp-t8/shot-f1-twopass-{libra,intree}.png`). One-pass `OriginalHistory1` probe confirms the geometry fix: `/tmp/ppsspp-t8/shot-f1-hist-libra.png` shows the whole frame (84.2% different from the old corner-bug capture `shot-hist-libra.png`, 21.5% vs the full-res in-tree `shot-hist-intree.png` - blur only). `stock` re-verified bit-identical after the fix (0 / 2088960 pixels, `shot-f1-stock-{libra,intree}.png`). |
