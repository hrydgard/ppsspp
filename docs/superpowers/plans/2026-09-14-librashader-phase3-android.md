...
- **Device facts (AYN Thor, serial 64dc3c35, Adreno 740):** memstick is on the SD card, config `/storage/9C33-6BBD/ROMs/psp/PSP/SYSTEM/ppsspp.ini` (writable via `adb shell`, edit with `adb shell sed -i` or pull/edit/push); user slang dir `/storage/emulated/0/Android/data/org.ppsspp.ppsspp/files/slang/` with the full libretro pack (`presets/crt-royale-downsample.slangp`, `presets/handheld-plus-color-mod/lcd-grid-v2-psp-color.slangp`, `crt/crt-maximus-royale-fast-mode.slangp`, ...). Current keys: `GraphicsBackend = 3 (VULKAN)`, `InternalResolution = 4`, `G3DLevel = 2` (set `G3DLevel = 4` and `SYSTEMLevel = 4` in BOTH the `[Log]` and `[LogDebug]` sections of the device ini (or Developer Tools → Logging channels) so INFO lines such as `librashader loaded` and `Slang chain backend:` reach logcat; restore afterwards). Launch a game: `adb shell am start -n org.ppsspp.ppsspp/.PpssppActivity --es org.ppsspp.ppsspp.Shortcuts 'content://com.android.externalstorage.documents/tree/9C33%2D6BBD%3AROMs%2Fpsp/document/9C33%2D6BBD%3AROMs%2Fpsp%2F3rd%20Birthday%2Eiso'`. Wake/unlock first (`adb shell input keyevent KEYCODE_WAKEUP; adb shell input keyevent 82`); the device sleeps quickly. Screenshots: `adb exec-out screencap -p > file.png` (exact framebuffer; compare with `/tmp/ppsspp-t8/cmp.py`). Logs: `adb logcat -c` before a run, `adb logcat -d | grep -E "PPSSPP|librashader|Fatal|DEBUG"` after. (Task 3 found PPSSPP receives a native Adreno OpenGL ES 3.2 context; the ANGLE string comes from SurfaceFlinger, not from PPSSPP's EGL context).
...
| Validation layers: CALLBACK/librashader messages | PASS (zero) | Messages arrive as `VKDEBUG:` through PPSSPP's debug callback (core validation only: `VK_LAYER_KHRONOS_validation` default features; the debug callback reports messages from enabled features, not synchronization validation, which PPSSPP does not enable). In-tree baseline: 6 (lcd-grid) / 10 (crt-royale-downsample) messages, all one ID — `WARNING(perf:-937765618) vkCreateGraphicsPipelines(): ... Vertex attribute at location 2 not consumed by vertex shader` (pre-existing PPSSPP noise from the in-tree slang pipelines). librashader on the same two presets: **0 VKDEBUG lines**. Set difference (librashader − baseline) = ∅; no VUID error, nothing attributable to the CALLBACK step. |
...
| Performance crt-royale-downsample (FPS librashader / in-tree) | PASS | FPS/speed counter (`iShowStatusFlags = 6`), two samples 10 s apart each: librashader **30/30 (100.0%) / 30/30 (100.0%)**, in-tree **30/30 (100.0%) / 30/30 (100.0%)** — the counter is vsync-capped at the game's 30 fps so it cannot discriminate. `DebugOverlay = 6` (GPU_PROFILE) gives single-frame overlay samples: librashader `CALLBACK librashader: 10.566 ms`, totalGPUTimeMs 14.36; in-tree 14 `RENDER slang-pass` + 1 BLIT = **8.634 ms**, totalGPUTimeMs 11.27 (+22% post-process cost for librashader; but unrelated steps in the same two captures differed by a similar magnitude — `MS_RENDER_DEPTH` 3.513 vs 2.333 ms — so this is an indication, not a measurement); both far inside the 33.3 ms budget. librashader also writes fewer descriptors (5 vs 19) and shows 0 resource deletions/frame (vs 14). Averaged measurement is a Phase 4 item. |
