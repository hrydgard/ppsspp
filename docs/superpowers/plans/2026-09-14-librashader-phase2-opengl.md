...
- **GL thread rule:** `libra_gl_filter_chain_*` only inside a native callback (GL thread, context current). `libra_gl_filter_chain_free` requires the creating context current → free via a CALLBACK step; at `DeviceLost`/shutdown drop the state without calling free and log once (GL objects died with the context; the Rust-side memory is leaked knowingly). `QueueFree` takes a `deviceLost` flag for this: `GLRenderManager::ThreadEnd` deletes queued CALLBACK functions without running them, so enqueuing a free during teardown would silently do nothing.
...
| srgb GL vs Vulkan | **PASS — bit-identical** | Added in the final fix wave (F9), HEAD `304750f031`. `./runp2.sh f9-srgb-gl $P/srgb.slangp True 0` and `./runp2.sh f9-srgb-vk $P/srgb.slangp True 3`, then `python3 cmp.py shot-f9-srgb-gl.png shot-f9-srgb-vk.png` → `pixels 2088960  differing 0 (0.0000%)  max 0`; both PNGs MD5 `19a7134257fdf55ede8611a8118ad5b4`. Both logs: `librashader loaded (ABI 2, API 5)`, `Slang chain backend: librashader`, `preset parsed: …/srgb.slangp`, no `LibrashaderFilterChain:`/`LibrashaderRuntimeOpenGL:` error and one pre-existing `E[System]` (`Exception handler not implemented`). So `srgb_framebuffer0 = true` behaves identically on both backends. |
...
| Chain free / shutdown | **NOT RUN — in-process teardown and `gl_filter_chain_free` not exercised; shutdown path drops the chain by design (see F1)** | The original PASS was wrong: it read the *absence* of the drop warning as proof the free ran, but at that point `DeviceLost()` enqueued a free CALLBACK that `GLRenderManager::ThreadEnd` deleted without running — so `gl_filter_chain_free` never ran and the drop-and-log fallback was unreachable (`draw` was still non-null). F1 makes the shutdown path drop the state explicitly: the GL runs after the fix log exactly one `LibrashaderRuntimeOpenGL: dropping chain without freeing (GL objects die with the context; librashader’s Rust-side allocation is knowingly leaked)` at exit, with no GL/driver error and a clean `Leaving main`. `gl_filter_chain_free` is now only reachable from an in-process preset change, which is still NOT RUN (needs synthetic input). |
